...
<div class="mt-3 pt-3 container">
    ...
    {{ Form::open(['route' => $isEdit ? ['admin.categories.update', $data->id] : 'admin.categories.store', 'method' => $isEdit ? 'put' : 'post']) }}

    <div class="card shadow-sm">
        <div class="card-body">
            <div class="mb-3">
                <label class="form-label">Empresa (vazio = global)</label>
                <select name="company_id" class="form-select" {{ $readonly ? 'disabled' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select name="parent_id" class="form-select" {{ $readonly ? 'disabled' : '' }}>
                    ...
                    @foreach($parents as $p)
                        <option value="{{ $p->id }}" {{ old('parent_id', $data->parent_id ?? '') == $p->id ? 'selected' : '' }}>
                            {{ $p->name }} (nível 1)
                        </option>
                        @foreach($p->children as $child)
                            <option value="{{ $child->id }}" {{ old('parent_id', $data->parent_id ?? '') == $child->id ? 'selected' : '' }}>
                                — {{ $child->name }} (nível 2)
                            </option>
                        @endforeach
                    @endforeach
                </select>
                <small class="text-muted">Máximo 3 níveis: raiz → sub → sub-sub</small>
            </div>
    ...
    <div class="card shadow-sm mt-4">
        <div class="card-header d-flex justify-content-between">
            ...
            @if($data->canHaveChildren())
                <a href="{{ route('admin.categories.create', ['type' => $data->type, 'parent_id' => $data->id]) }}" class="btn btn-sm btn-primary">Nova Subcategoria</a>
            @endif
        </div>
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-6 col-sm-4 d-flex justify-content-center gap-1">
                    <a href="{{ route('admin.categories.show', $child->id) }}" class="btn btn-sm btn-info">Ver</a>
                    <a href="{{ route('admin.categories.edit', $child->id) }}" class="btn btn-sm btn-primary">Editar</a>
                    {{ Form::open(['route' => ['admin.categories.destroy', $child->id], 'class' => 'd-inline']) }}
                    <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Excluir subcategoria?')">Excluir</button>
                    {{ Form::close() }}
                    {{ Form::open(['route' => ['admin.categories.update', $child->id], 'method' => 'put', 'class' => 'd-inline']) }}
                    <input type="hidden" name="name" value="{{ $child->name }}">
                    <input type="hidden" name="type" value="{{ $child->type }}">
                    <input type="hidden" name="parent_id" value="">
                    <button type="submit" class="btn btn-sm btn-outline-warning" onclick="return confirm('Remover vinculo com esta categoria?')">Desvincular</button>
                    {{ Form::close() }}
                </div>
                <div class="col-12 p-0"><hr class="p-0 m-1"></div>
                @endforeach
